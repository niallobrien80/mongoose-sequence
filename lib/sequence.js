 ...
 * @param {string} [options.start_at] Define a custom starting number for the sequence
...
Sequence.prototype._setNextCounter = function(doc, callback) {    
    ...
    this._counterModel.findOne(
        { id: id, reference_value: referenceValue },
        function(err, counter) {
            if (err) return callback(err);

            var m = t._counterModel();
            var update = counter ? { $inc: { seq: 1 } } : { seq : m.seq };            

            t._counterModel.findOneAndUpdate(
                { id: id, reference_value: referenceValue },
                update,
